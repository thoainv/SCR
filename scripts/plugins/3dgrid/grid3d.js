...
;( function( window ) {
	...
	grid3D.prototype._showContent = function( pos ) {
		...
		var rec = currentItem.getBoundingClientRect();
		this.placeholder.style.left = rec.left + 'px';
		this.placeholder.style.top = rec.top + 'px';
		this.placeholder.style.width = rec.width + 'px';
		this.placeholder.style.height = rec.height + 'px';
		debugger;
		// append placeholder to the grid
		this.gridWrap.parentElement.parentElement.parentElement.parentElement.appendChild( this.placeholder );
		self.placeholder.onclick = function () {
		    debugger;
		    self._hideContent();
		}
		// and animate it
		var animFn = function() {
			...
			setTimeout(function () {
			    classie.addClass(self.placeholder, "show");
			}, 1)
